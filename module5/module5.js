...
var spotLightDir = new THREE.Vector3(0,0,-1);
var uniforms;
...
$(function(){
	...
	uniforms = { 
		...
		dirlightColor: 
		{
		type: 'v4', value: colorToVec4(directionalLight.color)
	    },
	    spotlightDistance:
		{
		type: 'f', value: distance
	    },
	    spotlightPos:
		{
		type: 'v3', value: camObject.position
	    },
	    spotlightExponent:
		{
		type: 'f',value: spotLight.exponent
	    },
	    spotlightDirection:
		{
		type: 'v3', value: spotLightDir
	    },
	    spotlightIntensity:
		{
		type: 'f', value: 2.0 
		...
		spotlightColor: 
		{
		type: 'v4', value: colorToVec4(spotLight.color)
	    },
	    ambientColor:
		{ 
		type: 'v4', value: colorToVec4(ambientLight.color) /* global ambient */
	    }
	}
	...
	new THREE.MeshPhongMaterial({ color: 0xFF00FF, transparent: true})
    );
    ...
    thumb =  new THREE.Mesh( new THREE.CubeGeometry(0.05,0.25,0.05),
			     new THREE.MeshPhongMaterial({ color: 0xFFAAAA, transparent: true}));
    hand.add(thumb);
...
function update(){
	...
	//need to update the spot light direction vector and position
	uniforms.spotlightDirection.value = camera.localToWorld(spotLightObj.position.clone()).sub(spotLight.position).negate();
	uniforms.spotlightPos.value = camObject.position;
